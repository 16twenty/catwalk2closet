<div class="contact">
	<div class="row">
		<div class="six columns centered">
			<div class="ribbon">
				<h2>Contact Us</h2>
			</div>
		</div>
	</div>
	<div class="row contact_inner">
		<div class="four columns">
			<!--<div class="box" id="about">
				<a>Designers & Exhibitors</a>
				<span class="paint_pink"></span>
			</div>-->
			<h5 class="text-center">Designers & Exhibitors</h5>
			<p><i class="icon-mail"></i>Email: <a href="mailto:vedors@catwalk2closet.com">vedors@catwalk2closet.com</a></p>
			<p><i class="icon-phone"></i>Phone: <a href="tel:4165353939">(416) 555-1324</a></p>
		</div>
		<div class="four columns">
			<!--<div class="box" id="exhibitors">
				<a style="padding-top:110px;">Medias</a>
				<span class="paint_blue"></span>
			</div>-->
			<h5 class="text-center">Medias</h5>
			<p><i class="icon-mail"></i>Email: <a href="mailto:medias@prdepartment.ca">medias@prdepartment.ca</a></p>
			<p><i class="icon-phone"></i>Phone: <a href="tel:4165353939">(416) 535-3939</a></p>
		</div>
		<div class="four columns">
			<!--<div class="box" id="vip">
				<a>General Information</a>
				<span class="paint_pink"></span>
			</div>-->
			<h5 class="text-center">General Information</h5>
			<p><i class="icon-mail"></i>Email: <a href="mailto:general@catwalk2closet.com">general@catwalk2closet.com</a></p>
			<p><i class="icon-phone"></i>Phone: <a href="tel:4165353939">(416) 555-1234</a></p>
		</div>
	</div>
</div>
